...
export default function EditarProducto() {
  ...
  const verificarVendedorYCargar = async () => {
    try {
      ...
      if (!token || rol !== 'VENDEDOR') {
        Alert.alert('Acceso restringido', 'Debes ser vendedor para editar productos');
  ...
  const cargarProducto = async (token: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const productoNormalizado = {
          ...
          precioProducto: data.precioProducto ? data.precioProducto.toString() : '0',
          stockProducto: data.stockProducto ? data.stockProducto.toString() : '0',
          unidad: data.unidad || 'kg',
          idCategoria: data.subcategoria?.idCategoria?.toString() || '',
          idSubcategoria: data.subcategoria?.idSubcategoria?.toString() || '',
        };
        
        setForm(productoNormalizado);
        setImagenOriginal(data.imagenProducto);
        setImageUri(data.imagenProducto);
        
        if (productoNormalizado.idCategoria) {
          await cargarSubcategoriasPorCategoria(parseInt(productoNormalizado.idCategoria));
        }
      } else {
  ...
  const cargarSubcategoriasPorCategoria = async (idCategoria: number) => {
    try {
      const token = await AsyncStorage.getItem('authToken');
      const response = await fetch(
        `${API_BASE_URL}/subcategorias/categoria/${idCategoria}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/productos/editar/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        Alert.alert(
          ...
          'Los cambios en tu producto han sido guardados exitosamente',
          [
            {
              text: 'Ver producto',
              onPress: () => {
    ...
    <SafeAreaView style={styles.container}>
          ...
          <View style={styles.imageSection}>
            ...
            >
              {imageUri ? (
                ...
                <View style={styles.uploadPlaceholder}>
                  ...
                  <Text style={styles.uploadText}>Toca para cambiar la imagen</Text>
            ...
            >
              ...
              <Text style={styles.changeImageText}>
                {imageUri ? 'Cambiar imagen' : 'Cambiar imagen'}
              </Text>
            </TouchableOpacity>
          </View>
      ...
      {guardando && (
        <View style={styles.loadingOverlay}>
          <ActivityIndicator size="large" color="#FF6B35" />
          <Text style={styles.loadingOverlayText}>Actualizando producto...</Text>
        </View>
      )}
    </SafeAreaView>
  );
